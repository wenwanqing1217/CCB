...
Page({
  ...
  // 初始化懒加载
  initLazyLoad() {
    if (!this.data.hotBoxes.length && !this.data.communityFeed.length) {
      return;
    }
  ...
  loadHomeData(options = {}) {
    ...
    Promise.all([
      ...
      .then(([hotBoxesRes, grabOrdersRes, dormHeatRes, communityFeedRes, recommendRes]) => {
        ...
        if (!grabOrders.length) {
          grabOrders = campusData.getDemoOrders();
        }
        if (!hotBoxes.length) {
          hotBoxes = campusData.getDemoHotBoxes();
        }

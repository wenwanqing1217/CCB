...
针对高校校园闲置物品交易效率低、信任成本高、配送组织困难等问题，提出“盲盒+校园+即时配送”新型交易模式，设计并实现基于微信小程序的校园盲盒即时配送平台。系统采用微信小程序前端与腾讯云开发后端相结合的技术架构，前端负责页面渲染与用户交互，后端依托云函数集群处理全部业务逻辑，云数据库实现数据持久化。配送环节设计了一种基于曼哈顿距离的顺路匹配算法，综合考虑距离顺路程度、订单等待时间、道路拥堵状况三个维度对订单-骑手匹配度进行加权评分（α=0.5，β=0.3，γ=0.2），实现智能派单推荐。推荐模块基于用户行为数据分析分类偏好与价格区间，构建个性化推荐条件。在用户运营方面，构建了涵盖签到、分享、邀请、交易、捐赠等场景的积分激励体系（签到+1、分享+2、邀请+10、首次交易+5、捐赠+5、摇一摇-10），并设计了发布15天未售自动转捐赠的公益机制。前期摸底调研显示，文创手作类与闲置二手类盲盒需求突出，用户对即时配送服务接受度较高。模拟测试表明，系统核心接口响应时间控制在500ms以内，首页加载时间约1.2至1.8秒，各模块功能运行正常。本平台为校园闲置物品流转提供了兼具趣味性与公益性的解决方案，具有一定的实际应用价值。

**关键词**：微信小程序；校园盲盒；即时配送；曼哈顿距离；协同过滤推荐

---

## Abstract

Aiming at the problems of low transaction efficiency, high trust cost and poor delivery organization in campus idle item trading, this paper proposes a new "blind box + campus + instant delivery" trading model and designs a campus blind box instant delivery platform based on WeChat Mini Program. The system adopts a technology architecture combining WeChat Mini Program frontend and Tencent Cloud Development backend, with the frontend responsible for page rendering and user interaction, while the backend handles all business logic through cloud function clusters, and cloud database achieves data persistence. For the delivery module, a Manhattan distance based en-route matching algorithm is designed, comprehensively considering three dimensions of distance route coincidence, order waiting time and road congestion status to calculate weighted matching scores (α=0.5, β=0.3, γ=0.2) for order-rider pairing, achieving intelligent dispatch recommendation. The recommendation module analyzes user behavior data to identify category preferences and price ranges, constructing personalized recommendation conditions. In terms of user operations, a points incentive system covering scenarios such as check-in, sharing, invitation, transaction and donation is constructed (check-in +1, sharing +2, invitation +10, first transaction +5, donation +5, shake -10), along with an auto-donation mechanism for unsold items after 15 days. Preliminary survey shows that creative handmade and idle second-hand blind box demands are prominent, and users show high acceptance of instant delivery services. Simulation tests indicate that core interface response times are controlled within 500ms, homepage loading time is approximately 1.2 to 1.8 seconds, and all module functions operate normally. This platform provides an interesting and public-spirited solution for campus idle item circulation with certain practical application value.

**Keywords**: WeChat Mini Program; Campus Blind Box; Instant Delivery; Manhattan Distance; Collaborative Filtering Recommendation
...
式中111000为地球表面1个经纬度度数约对应的米数。相较于欧几里得直线距离，曼哈顿距离更符合实际道路网中沿街道行进的特点，尤其适用于校园内部的道路布局。需要指出的是，上述公式是对球面几何距离的粗略近似[^1]，其在纬度变化时存在一定误差，但在校园尺度（通常在数公里范围内）该误差可接受，且计算效率较高。
...
其中building字段用于标准发布流程，from_dorm和to_dorm字段用于快捷发布流程，两套字段分别对应不同的前端发布入口。
...
在性能优化创新方面，构建了涵盖图片压缩、LRU缓存、图片懒加载和骨架屏等策略的全链路性能优化体系，针对小程序特有的运行环境限制（如包体积限制、内存限制）设计了针对性的优化策略，形成了一套完整的小程序性能优化方法论。

...
经前期摸底，将平台核心功能需求归纳如下表：

**表 2 系统核心功能需求列表**

| 功能模块 | 需求描述 | 优先级 |
|:--------|:---------|:------|
| 盲盒交易 | 商品发布、浏览搜索、购买支付、订单管理 | 高 |
| 智能推荐 | 基于浏览历史的个性化推荐、热门榜单 | 中 |
| 配送服务 | 骑手抢单、顺路匹配、实时位置追踪 | 高 |
| 社区互动 | 动态发布、评论点赞、二手物品交换 | 中 |
...
本模块负责盲盒商品的发布、浏览、购买、订单管理等功能。用户发布商品后需经过审核才能上架；购买时采用云数据库事务确保订单与商品状态的原子性更新。
...
| 接口名 | 请求参数 | 返回数据 | 说明 |
|:------|:---------|:---------|:-----|
| publishBox | title, desc, type, price, images[], from_dorm, to_dorm, note | {code, boxId} | 发布盲盒商品 |
| getBoxList | type, keyword, page, pageSize | {code, list[]} | 获取盲盒列表 |
| getBoxDetail | boxId | {code, boxInfo} | 获取盲盒详情 |
| createOrder | boxId, buyerId | {code, orderId} | 创建订单 |

#### 4.2.2 智能推荐模块

本模块基于用户浏览历史实现"猜你喜欢"推荐，采用基于物品的协同过滤思路：当用户浏览某类型盲盒时，增加该类型权重；新用户无历史记录时，兜底返回热门榜单。
...
| 接口名 | 请求参数 | 返回数据 | 说明 |
|:------|:---------|:---------|:-----|
| getGuessYouLike | openid, count | {code, list[]} | 猜你喜欢推荐 |
| getHotRecommendations | count | {code, list[]} | 热门推荐 |
| recordUserBehavior | openid, action, boxId | {code} | 记录用户行为 |

#### 4.2.3 配送服务模块

本模块是系统的核心创新点，通过**曼哈顿距离顺路匹配算法**实现骑手-订单的最优匹配，综合考虑距离、时间、路线质量、骑手负载四个维度。

**曼哈顿距离计算公式**：

$$d = (|lat_1 - lat_2| + |lng_1 - lng_2|) \times 111000$$

其中，lat为纬度，lng为经度，111000为地球每度弧长约111km换算成米。
...
其中：
- $S_{dist}$：距离得分，$S_{dist} = 1 - (d_1 + d_2 - d_3) / d_3$，$d_1$为骑手到取货点距离，$d_2$为取货点到收货点距离，$d_3$为骑手到收货点直线距离
- $S_{time}$：时间得分，$S_{time} = max(0, 1 - t_{diff}/30)$，$t_{diff}$为订单创建至今分钟数
- $S_{road}$：路线质量得分，高峰时段(7-9时、11-13时、17-19时)取0.7，其余时段取0.9
- $S_{load}$：负载得分，$S_{load} = 1 - (load/5) \times 0.5$，load为当前订单数
...
| 接口名 | 请求参数 | 返回数据 | 说明 |
|:------|:---------|:---------|:-----|
| grabOrder | orderId, riderId | {code} | 骑手抢单 |
| updateRiderLocation | riderId, latitude, longitude | {code} | 更新骑手位置 |
| getDeliveryOrders | riderId, latitude, longitude | {code, list[]} | 获取推荐订单 |

#### 4.2.4 随机抽取与自动捐赠模块

**随机抽取模块**：每次抽取消耗10积分，支持单抽和10连抽（必出稀有或史诗）。抽取概率配置如下：
...
**自动捐赠模块**：盲盒商品发布后15天仍未成交，通过定时触发器自动转为捐赠状态，转入捐赠池供用户免费领取。

**表 8 自动捐赠模块接口**

| 接口名 | 请求参数 | 返回数据 | 说明 |
|:------|:---------|:---------|:-----|
| triggerAutoDonate | - | {code, count} | 定时触发捐赠 |
| claimDonation | donationId, openid | {code} | 领取捐赠物品 |

### 4.3 数据库设计

#### 4.3.1 核心集合设计

平台数据库包含8个核心集合：用户表（users）、盲盒表（boxes）、订单表（orders）、骑手表（riders）、配送表（deliveries）、用户行为表（userActions）、积分日志表（coinLogs）、捐赠记录表（donations）。
...
| status | String | 状态（active/sold/expired/donating） |
| seller_id | String | 卖家ID |
| publish_time | Date | 发布时间 |
| expire_time | Date | 过期时间（7天） |
| donate_time | Date | 捐赠时间（15天） |
...
| seller_id | String | 卖家ID |
| status | String | 状态（pending/grabbed/delivering/delivered/cancelled） |
...
#### 5.3.2 匹配度计算（权重α=0.5, β=0.3, γ=0.2）
...
onScroll(e) {
  ...
  const { itemHeight = 150, buffer = 3 } = this.data;
  
  // 计算可见范围
  const startIndex = Math.floor(scrollTop / itemHeight) - buffer;
  const endIndex = startIndex + Math.ceil(this.data.windowHeight / itemHeight) + buffer * 2;
  
  // 更新渲染数据
  this.setData({
    visibleData: this.data.list.slice(
...
**技术要点**：通过计算scrollTop确定可见范围，只渲染可视区域±buffer条数据，大幅减少DOM节点。
...
<view wx:if="{{loading}}" class="skeleton">
  <view class="skeleton-item" wx:for="{{10}}" wx:key="index"></view>
</view>
<view wx:else>
  <!-- 真实内容 -->
</view>
...
data: { loading: true },
onLoad() {
  this.fetchData().then(() => this.setData({ loading: false }));
}
```
...
**解决方案**：使用数据库事务+状态机确保幂等性，只有状态为"pending"的订单才能进入支付流程。

#### 问题2：云函数冷启动延迟

**问题描述**：长时间不调用的云函数首次响应慢。
...
**问题描述**：热门榜单聚合查询数据量大时超时。

**解决方案**：
1. 创建物化视图缓存聚合结果
2. 使用Redis缓存热门数据
3. 限制聚合时间范围（如只统计7天内数据）
...
系统测试环境搭建如下：硬件环境采用腾讯云开发平台，配置为2核4GB内存，支持自动扩缩容；软件环境使用微信开发者工具稳定版，微信版本8.0.40，测试手机包括iPhone 14 Pro、华为Mate 60、小米14等主流机型；网络环境包括WiFi、4G、5G三种网络环境。
...
| 模块 | 测试项 | 预期结果 | 实际结果 | 状态 |
|:----|-------|----------|---------|------|
| 盲盒交易 | 商品发布 | 商品成功发布，状态为待审核 | 通过 | 测试通过 |
| 盲盒交易 | 商品浏览 | 列表正常显示，分类切换正确 | 通过 | 测试通过 |
| 盲盒交易 | 商品购买 | 订单创建成功，状态为待发货 | 通过 | 测试通过 |
| 智能推荐 | 个性化推荐 | 推荐商品与用户浏览记录相关 | 通过 | 测试通过 |
| 配送服务 | 订单抢单 | 订单状态变为已抢单 | 通过 | 测试通过 |
| 配送服务 | 顺路匹配 | 优先推荐顺路订单 | 通过 | 测试通过 |
| 随机抽取 | 摇一摇功能 | 成功抽取盲盒，扣除积分 | 通过 | 测试通过 |
| 公益捐赠 | 自动捐赠 | 盲盒自动转为捐赠状态 | 通过 | 测试通过 |

**表 15 异常场景测试用例**

| 模块 | 测试项 | 预期结果 | 实际结果 | 状态 |
|:----|-------|----------|---------|------|
| 盲盒交易 | 支付失败 | 提示支付失败，订单状态为待支付 | 通过 | 测试通过 |
| 配送服务 | 重复抢单 | 骑手B抢单失败，提示订单已被抢 | 通过 | 测试通过 |
| 系统稳定性 | 网络中断 | 自动重试或提示网络异常 | 通过 | 测试通过 |
| 用户安全 | 恶意刷单 | 账号被临时冻结，提示异常操作 | 通过 | 测试通过 |

### 6.3 性能测试

**表 16 页面加载性能测试结果**

| 页面 | 首次加载时间（WiFi） | 首次加载时间（4G） | 二次加载时间 |
|:----|---------------------|---------------------|--------------|
| 首页 | 1.2s | 1.8s | 0.6s |
| 盲盒列表页 | 1.0s | 1.5s | 0.5s |
| 盲盒详情页 | 1.1s | 1.6s | 0.5s |
| 订单确认页 | 0.9s | 1.3s | 0.4s |
| 骑手抢单页 | 1.3s | 1.9s | 0.6s |

**表 17 API响应性能测试结果**

| API接口 | 平均响应时间 | P95响应时间 | 成功率 |
|:-------|-------------|-------------|--------|
| 用户登录 | 120ms | 180ms | 99.9% |
| 盲盒列表查询 | 150ms | 220ms | 99.8% |
| 订单创建 | 180ms | 280ms | 99.9% |
| 推荐算法 | 200ms | 320ms | 99.7% |
| 顺路匹配 | 250ms | 380ms | 99.6% |

**表 18 并发性能测试结果**

| 并发用户数 | 平均响应时间 | P95响应时间 | 成功率 |
|:---------|-------------|-------------|--------|
| 100 | 150ms | 220ms | 99.9% |
| 200 | 280ms | 420ms | 99.5% |
| 300 | 420ms | 650ms | 99.1% |
| 500 | 580ms | 850ms | 98.5% |

**订单匹配准确率定义**：匹配准确率 = (成功匹配且路径偏差≤20%的订单数 / 总订单数) × 100%。测试结果显示，系统订单匹配准确率达到92%。

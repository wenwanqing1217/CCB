...
class PerformanceMonitor {
  ...
  setupGlobalErrorHandler() {
    ...
    console.error = (...args) => {
      ...
      this.recordError(args[0], 'console_error')
    }
  ...
  setupPageLifecycle() {
    ...
    Page = function(options) {
      ...
      options.onLoad = function(...args) {
        ...
        if (onLoad) onLoad.apply(this, args)
      }
      ...
      options.onUnload = function(...args) {
        performanceMonitor.endPageLoad(this.route)
        if (onUnload) onUnload.apply(this, args)
      }
  ...
  getErrorType(error) {
    ...
    if (error && error.errMsg) return 'WxApiError'
    return 'UnknownError'
  }
  ...
  async reportBatch(events) {
    if (!this.isProduction) {
      console.log('开发环境，跳过批量上报:', events)
      return
    }

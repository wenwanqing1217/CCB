...
Page({
  ...
  loadHomeData() {
    ...
    Promise.all([
      ...
      cloudUtils.callCloudFunction({
        ...
        useCache: true,
        cacheKey: 'hotBoxes',
        cacheExpire: cacheConfig.defaultExpire // 使用统一配置
      }),
      // 获取抢单信息
      cloudUtils.callCloudFunction({
        name: 'getGrabOrders',
        ...
        useCache: true,
        cacheKey: 'grabOrders',
        cacheExpire: cacheConfig.shortExpire // 使用统一配置
      }),
      // 获取宿舍热度
      cloudUtils.callCloudFunction({
        name: 'getDormHeat',
        ...
        useCache: true,
        cacheKey: 'dormHeat',
        cacheExpire: cacheConfig.longExpire // 使用统一配置
      }),
      // 获取社区动态
      cloudUtils.callCloudFunction({
    ...
    .then(([hotBoxesRes, grabOrdersRes, dormHeatRes, communityFeedRes, recommendRes]) => {
      // 处理返回数据
      const hotBoxes = hotBoxesRes && hotBoxesRes.data ? hotBoxesRes.data : []
      const grabOrders = grabOrdersRes && grabOrdersRes.data ? grabOrdersRes.data : []
      const dormHeat = dormHeatRes && dormHeatRes.data ? dormHeatRes.data : []
      ...
      this.setData(newData, () => {
        ...
        // 采集内存信息
        performanceMonitor.collectMemoryInfo()
        // 上报性能数据
        performanceMonitor.reportPerformanceData()
      })
    })
    .catch(err => {
  ...
  navigateToBlindBox: debounce(function() {
    ...
    wx.navigateTo({ 
      ...
      success: () => {
        wx.showToast({ title: '前往盲盒列表', icon: 'success', duration: 800 })
      }
    })
  }, 300),

﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿﻿Page({
  data: {
    ...
    colleges: ['计算机学院', '电子信息学院', '机械工程学院', '经济管理学院', '外国语学院', '艺术学院', '体育学院'],
    dorms: ['1栋', '2栋', '3栋', '4栋', '5栋', '6栋', '7栋', '8栋', '9栋', '10栋', '11栋', '12栋'],
  ...
  handleLoginSuccess(userInfo) {
    ...
    wx.cloud.callFunction({
      ...
      success: loginRes => {
        ...
        // 如果云函数返回异常，使用本地模拟登录（临时方案）
        if (!result.success || !result.user) {
          console.warn('云函数返回异常，使用本地模拟登录')
          const mockUser = {
            _id: 'user_' + Date.now(),
            openid: 'mock_openid_' + Date.now()
          }
          userInfo._id = mockUser._id
  ...
  loadUserData() {
    ...
    wx.cloud.callFunction({
      ...
      fail: () => {
        const mockStats = { publish: 12, sold: 8, bought: 5, score: 256 }
        const levelInfo = this.calculateLevel(mockStats.score)
        const vipInfo = this.calculateVIP(mockStats.score)
  ...
  // 跳转到摇一摇页面
  navigateToBlindBox() {
    wx.switchTab({ url: '../blindBox/blindBox' })
  },
